...
main.data2<-cbind(tru.est,cox.est,exp.est,weib.est,cox.rmse,exp.rmse,weib.rmse,cox.cp,exp.cp,weib.cp)
colnames(main.data2)<-c("true.x0","true.x1","true.z0","true.z1","true.z2","true.p","cen.lat","cen.obs",
                       "cox.x1","cox.x1.se",
...
for(i in 1:nsims){
  ...
  X<-cbind(1,x)
  Z<-cbind(1,z,x)
  BayesZExponential = mcmcOF(Y, C, X, Z, N = 400, burn = 100, thin = 1,  w = c(1, 1, 1), m = 10, form = "Exponential")
  ...
  X<-cbind(1,x)
  Z<-cbind(1,z,x)
  BayesZWeibull = mcmcOF(Y, C, X, Z, N = 400, burn = 100, thin = 1,  w = c(1, 1, 1), m = 10, form = "Weibull")
  ...
  main.data2[i, ]<-c(tru.est[i, ],cox.est[i, ],exp.est[i, ],weib.est[i, ],cox.rmse[i, ],exp.rmse[i, ],weib.rmse[i, ],
                    cox.cp[i, ],exp.cp[i, ],weib.cp[i, ])
  
}
#save dataset
main.data2<-as.data.frame(main.data2)
