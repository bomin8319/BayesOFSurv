...
library(mcmcse)
library(devtools)
install_github('minniejoo/BayesMFSurv',force=TRUE)
library(BayesMFsurv)
library(MCMCpack)
library(plyr)
library(mvtnorm)
#set working directory
setwd("C:/Users/happy/Documents/GitHub/BayesOFsurv/coding material/Monte Carlos/Mixture DGP")
...
colnames(main.data)<-c("true.x0","true.x1","true.z0","true.z1","true.z2","true.p","cen.lat","cen.obs",
                       ...
                       "bzwei2.x0","bzwei2.x0.se","bzwei2.x1","bzwei2.x1.se","bzwei2.z0","bzwei2.z0.se","bzwei2.z1","bzwei2.z1.se","bzwei2.z2","bzwei2.z2.se","bzwei2.p","bzwei2.p.se",
                       "cox.x1.rmse",
                       ...
                       "bzwei2.x0.rmse","bzwei2.x1.rmse","bzwei2.z0.rmse","bzwei2.z1.rmse","bzwei2.z2.rmse","bzwei2.p.rmse",
                       ...
                       "bzwei2.x0.cp","bzwei2.x1.cp","bzwei2.z0.cp","bzwei2.z1.cp","bzwei2.z2.cp","bzwei2.p.cp")
...
y <- rweibull(n, shape=2, scale = myrates) # generates the r.v.
cen <- rweibull(n, shape=2,scale = 1 )
ycen <- pmin(y, cen)
di <- as.numeric(y <= cen)
...
X<-cbind(1,x)
Z<-cbind(1,z,x)
BayesZWeibull = bayes.mfsurv.est(Y, C, X, Z, N = 400, burn = 100, thin = 1,  w = c(5, 5, 5), m = 100, form = "Weibull", na.action="na.fail")
output.BayesZWeibull = list(par = c(summary(mcmc(BayesZWeibull$betas))[[1]][,1], summary(mcmc(BayesZWeibull$gammas))[[1]][,1], 
                                    summary(mcmc(BayesZWeibull$lambda))[[1]][1]), 
                            se = c(summary(mcmc(BayesZWeibull$betas))[[1]][,4], summary(mcmc(BayesZWeibull$gammas))[[1]][,4], 
                                   summary(mcmc(BayesZWeibull$lambda))[[1]][4]),
                            CI = rbind(summary(mcmc(BayesZWeibull$betas))[[2]], summary(mcmc(BayesZWeibull$gammas))[[2]], 
...
X<-cbind(1,x)
Z<-cbind(1,z,x)
BayesZWeibull2 = bayes.mfsurv.est3(Y, C, X, Z, N = 400, burn = 100, thin = 1,  w = c(5, 5, 5), m = 100, form = "Weibull", na.action="na.fail")
output.BayesZWeibull2 = list(par = c(summary(mcmc(BayesZWeibull2$betas))[[1]][,1], summary(mcmc(BayesZWeibull2$gammas))[[1]][,1], 
                                    summary(mcmc(BayesZWeibull2$lambda))[[1]][1]), 
                            se = c(summary(mcmc(BayesZWeibull2$betas))[[1]][,4], summary(mcmc(BayesZWeibull2$gammas))[[1]][,4], 
                                   summary(mcmc(BayesZWeibull2$lambda))[[1]][4]),
                            CI = rbind(summary(mcmc(BayesZWeibull2$betas))[[2]], summary(mcmc(BayesZWeibull2$gammas))[[2]], 
                                       summary(mcmc(BayesZWeibull2$lambda))[[2]]))

weib.est[i,31]<-output.BayesZWeibull2$par[1]
weib.est[i,32]<-output.BayesZWeibull2$se[1]
weib.est[i,33]<-output.BayesZWeibull2$par[2]
weib.est[i,34]<-output.BayesZWeibull2$se[2]
weib.est[i,35]<-output.BayesZWeibull2$par[3]
weib.est[i,36]<-output.BayesZWeibull2$se[3]
weib.est[i,37]<-output.BayesZWeibull2$par[4]
weib.est[i,38]<-output.BayesZWeibull2$se[4]
weib.est[i,39]<-output.BayesZWeibull2$par[5]
weib.est[i,40]<-output.BayesZWeibull2$se[5]
weib.est[i,41]<-output.BayesZWeibull2$par[6]
weib.est[i,42]<-output.BayesZWeibull2$se[6]

#store rmse
weib.rmse[i,16]<-sqrt((tru.est[i,1]-weib.est[i,31])^2)
weib.rmse[i,17]<-sqrt((tru.est[i,2]-weib.est[i,33])^2)
weib.rmse[i,18]<-sqrt((tru.est[i,3]-weib.est[i,35])^2)
weib.rmse[i,19]<-sqrt((tru.est[i,4]-weib.est[i,37])^2)
weib.rmse[i,20]<-sqrt((tru.est[i,5]-weib.est[i,39])^2)
weib.rmse[i,21]<-sqrt((tru.est[i,6]-weib.est[i,41])^2)

#calculate upper and lower 95% CI's
b0.lower<-output.BayesZWeibull2$CI[1,1]
b0.upper<-output.BayesZWeibull2$CI[1,5]
b1.lower<-output.BayesZWeibull2$CI[2,1]
b1.upper<-output.BayesZWeibull2$CI[2,5]
g0.lower<-output.BayesZWeibull2$CI[3,1]
g0.upper<-output.BayesZWeibull2$CI[3,5]
g1.lower<-output.BayesZWeibull2$CI[4,1]
g1.upper<-output.BayesZWeibull2$CI[4,5]
g2.lower<-output.BayesZWeibull2$CI[5,1]
g2.upper<-output.BayesZWeibull2$CI[5,5]
p.lower<-output.BayesZWeibull2$CI[6,1]
p.upper<-output.BayesZWeibull2$CI[6,2]
# g0.lower<-weib.est[i,19]-(1.959964*weib.est[i,20])
# g0.upper<-weib.est[i,19]+(1.959964*weib.est[i,20])
# g1.lower<-weib.est[i,21]-(1.959964*weib.est[i,22])
# g1.upper<-weib.est[i,21]+(1.959964*weib.est[i,22])
...
#store coverage parameters
weib.cp[i,16]<-ifelse(tru.est[i,3]>g0.lower & tru.est[i,3]<g0.upper, 1,0)
weib.cp[i,17]<-ifelse(tru.est[i,4]>g1.lower & tru.est[i,4]<g1.upper, 1,0)
weib.cp[i,18]<-ifelse(tru.est[i,5]>g2.lower & tru.est[i,5]<g2.upper, 1,0)
weib.cp[i,19]<-ifelse(tru.est[i,1]>b0.lower & tru.est[i,1]<b0.upper, 1,0)
weib.cp[i,20]<-ifelse(tru.est[i,2]>b1.lower & tru.est[i,2]<b1.upper, 1,0)
weib.cp[i,21]<-ifelse(tru.est[i,6]>p.lower & tru.est[i,6]<p.upper, 1,0)
...
#save dataset
main.data1000_weibullDGP<-as.data.frame(main.data)
write.dta(main.data1000_weibullDGP,"main.data1000_weibullDGP.dta")
