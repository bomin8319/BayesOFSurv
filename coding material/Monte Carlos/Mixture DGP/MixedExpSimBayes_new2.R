...
main.data3<-cbind(tru.est,cox.est,exp.est,weib.est,cox.rmse,exp.rmse,weib.rmse,cox.cp,exp.cp,weib.cp)
colnames(main.data3)<-c("true.x0","true.x1","true.z0","true.z1","true.z2","true.p","cen.lat","cen.obs",
                       "cox.x1","cox.x1.se",
...
for(i in 1:nsims){
  ...
  ZExponential<- function(est,Y,C,X,Z,data) {      
    ...
    ZG<-Z%*%gamma
    phi<-1/(1+exp(-ZG))
    llik<-C*(log((1-phi)+phi*exp(XB)*exp(-exp(XB)*Y)))+(1-C)*(-exp(XB)*Y)
    llik<--1*sum(llik)
    return(llik)
  }
  ...
  ZWeibull<- function(est,Y,C,X,Z,data) {      
    ...
    ZG<-Z%*%gamma
    phi<-1/(1+exp(-(ZG+1/p)))
    llik<-C*(log((1-phi)+phi*exp(XB+1/p)*p*((exp(XB+1/p)*Y)^(p-1))*exp(-(exp(XB+1/p)*Y)^p)))+(1-C)*(-(exp(XB+1/p)*Y)^p)
    llik<--1*sum(llik)
    return(llik)
  }
  ...
  main.data3[i, ]<-c(tru.est[i, ],cox.est[i, ],exp.est[i, ],weib.est[i, ],cox.rmse[i, ],exp.rmse[i, ],weib.rmse[i, ],
                    cox.cp[i, ],exp.cp[i, ],weib.cp[i, ])
  
}
#save dataset
main.data3<-as.data.frame(main.data3)
write.dta(main.data3,"main.data3.dta", )